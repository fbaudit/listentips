...
export async function GET() {
  ...
  const { data: reportTypes, error } = await supabase
    ...
    .select("id, type_name, type_name_en, type_name_ja, type_name_zh, code, description, notes, display_order, is_active, created_at")
    .order("display_order");

  if (error) {
    return NextResponse.json({ error: "Failed to load default report types" }, { status: 500 });
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!body.type_name) {
    return NextResponse.json({ error: "유형명은 필수입니다" }, { status: 400 });
  }
