...
import { ArrowLeft, Send, Loader2, Download, FileText, Clock, Trash2 } from "lucide-react";
  ...
  const fetchReportData = async () => {
    try {
      ...
      if (reportRes.ok) {
        ...
        // Fetch statuses for this report's company
        if (data.report?.company_id) {
          const statusesRes = await fetch(`/api/admin/report-statuses?companyId=${data.report.company_id}`);
          if (statusesRes.ok) {
                        ...
                        >
                          <div className={`flex items-center gap-2 mb-1 ${isMe ? "justify-end" : ""}`}>
                            <span className="font-medium text-xs">
                              {c.author_type === "reporter" ? "제보자" : c.author_type === "super_admin" ? "플랫폼 관리자" : "관리자"}
                            </span>
